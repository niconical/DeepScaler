...
times_train = [
    ('2024-03-23 15:33:00', '2024-03-23 20:33:00')
]#

times_valid = [
    ('2024-03-23 17:33:00', '2024-03-23 22:33:00')
]#

times_test = [
    ('2024-03-24 02:30:00', '2024-03-24 08:30:00')
]#

# save_all_fetch_data(times_train, 1, root_dir='./data/train/', interval=30, services=services,metrics=metrics)#interval 间隔
# save_all_fetch_data(times_valid, 1, root_dir='./data/valid/', interval=30, services=services,metrics=metrics)#interval 间隔
save_all_fetch_data(times_test, 1, root_dir='./data/test/', interval=30, services=services,metrics=metrics)#interval 间隔
...
#                                         num_of_hours=4,  # 生成一个小时的数据作为验证集
#                                         num_for_predict=120,  # 预测未来 120 个数据点
#                                         points_per_hour=120,  # 每小时 120 个数据点
#                                         save=True)

# 生成测试集数据
test_data = read_and_generate_dataset(graph_signal_matrix_filename='./data/boutique/test.npz',
                                       num_of_hours=2,  # 生成两个小时的数据作为测试集
                                       num_for_predict=120,  # 预测未来 1 个数据点
                                       points_per_hour=120,  # 每小时 120 个数据点
                                       save=True)
