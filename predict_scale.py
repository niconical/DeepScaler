...
def main(args):
    k8s_op = K8sOp()
    svc_ls = ["adservice", "cartservice", "checkoutservice", "currencyservice", "emailservice",
    ...
    while True:
        ...
        if (c_temp == 0):
            start_time_str = (current_time-datetime.timedelta(hours=1) -
                              datetime.timedelta(minutes=21)).strftime('%Y-%m-%d %H:%M:%S')
        else:
            start_time_str = (
        ...
        save_all_fetch_data(times_original, 1, root_dir='/dataForPredict/',
                            interval=60, services=services)  # interval 间隔
        # pod
        # "adservice"
        ...
        for i in range(timeLen):
            listpod = np.vstack((adservice_pod[i], cartservice_pod[i], checkoutservice_pod[i], currencyservice_pod[i], emailservice_pod[i],
                                frontend_pod[i], paymentservice_pod[i], productcatalogservice_pod[i], recommendationservice_pod[i], shippingservice_pod[i]))
            listcpu = np.vstack((adservice_cpu[i], cartservice_cpu[i], checkoutservice_cpu[i], currencyservice_cpu[i], emailservice_cpu[i],
                                frontend_cpu[i], paymentservice_cpu[i], productcatalogservice_cpu[i], recommendationservice_cpu[i], shippingservice_cpu[i]))
            listres = np.vstack((adservice_res[i], cartservice_res[i], checkoutservice_res[i], currencyservice_res[i], emailservice_res[i],
                                frontend_res[i], paymentservice_res[i], productcatalogservice_res[i], recommendationservice_res[i], shippingservice_res[i]))
            listreq = np.vstack((adservice_req[i], cartservice_req[i], checkoutservice_req[i], currencyservice_req[i], emailservice_req[i],
                                frontend_req[i], paymentservice_req[i], productcatalogservice_req[i], recommendationservice_req[i], shippingservice_req[i]))
            listmem = np.vstack((adservice_mem[i], cartservice_mem[i], checkoutservice_mem[i], currencyservice_mem[i], emailservice_mem[i],
                                frontend_mem[i], paymentservice_mem[i], productcatalogservice_mem[i], recommendationservice_mem[i], shippingservice_mem[i]))
        ...
        all_data = predict_read_and_generate_dataset(
            graph_signal_matrix_filename='predict_scale.npz', num_of_hours=1, num_for_predict=1, points_per_hour=80, save=True)
        ...
        # ----------------------- Load data ------------------------
        Scaler = getattr(sys.modules['utils.scaler'],
                         train_config['scaler'])  # StandardScaler
        ...
        if ((float(checkoutservice)-math.floor(float(checkoutservice))) < restriction):
            pods_num_to_scale['checkoutservice'] = math.floor(
                float(checkoutservice))
        else:
            pods_num_to_scale['checkoutservice'] = math.ceil(
                float(checkoutservice))

        if ((float(currencyservice)-math.floor(float(currencyservice))) < 0.5):
            pods_num_to_scale['currencyservice'] = math.floor(
                float(checkoutservice))
        else:
            pods_num_to_scale['currencyservice'] = math.ceil(
                float(currencyservice))
        ...
        if ((float(paymentservice)-math.floor(float(paymentservice))) < 0.5):
            pods_num_to_scale['paymentservice'] = math.floor(
                float(paymentservice))
        else:
            pods_num_to_scale['paymentservice'] = math.ceil(
                float(paymentservice))

        if ((float(productcatalogservice)-math.floor(float(productcatalogservice))) < restriction):
            pods_num_to_scale['productcatalogservice'] = math.floor(
                float(productcatalogservice))
        else:
            pods_num_to_scale['productcatalogservice'] = math.ceil(
                float(productcatalogservice))

        if ((float(recommendationservice)-math.floor(float(recommendationservice))) < restriction):
            pods_num_to_scale['recommendationservice'] = math.floor(
                float(recommendationservice))
        else:
            pods_num_to_scale['recommendationservice'] = math.ceil(
                float(recommendationservice))
        if ((float(shippingservice)-math.floor(float(shippingservice))) < 0.5):
            pods_num_to_scale['shippingservice'] = math.floor(
                float(shippingservice))
        else:
            pods_num_to_scale['shippingservice'] = math.ceil(
                float(shippingservice))
        end = time.time()
        print(end-start)
        for svc in svc_ls:
            ...
            k8s_op.scale_deployment_by_replicas(
                svc, "boutiquessj", pods_num_to_scale[svc])
